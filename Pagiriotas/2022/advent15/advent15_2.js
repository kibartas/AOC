...
function mapImposibleRanges(sensors, beacons) {
  const impossibleRanges = new Map();
  for (let i = 0; i < sensors.length; i++) {
    const sensor = sensors[i];
    const beacon = beacons[i];

    const distance = calcDistance(sensor, beacon);
...
function isImposible(x, y, sensors, beacons) {
  if (x < 0 || x > maxCoordinate) return true;
  if (y < 0 || y > maxCoordinate) return true;
  for (let i = 0; i < sensors.length; i++) {
    const sensor = sensors[i];
    const beacon = beacons[i];
    const point = {
    ...
    const distanceToBeacon = calcDistance(sensor, beacon);
    const distanceToSensor = calcDistance(point, sensor);
...
function getDiamondEdges(x, y, distance) {
  const edges = [];
  for (let i = distance; i >= 0; i--) {
    edges.push({
      x: x + i + 1,
      y: y - Math.abs(distance - i),
    });
    edges.push({
      x: x - i - 1,
      y: y + Math.abs(distance - i),
...
function findBeacon(impossibleRanges, sensors, beacons) {
  for (let [sensor, distance] of impossibleRanges) {
    const edges = getDiamondEdges(sensor.x, sensor.y, distance);
    for (let i = 0; i < edges.length; i++) {
      const edge = edges[i];
      if (!isImposible(edge.x, edge.y, sensors, beacons)) {
        return edge;
      }
    }
  }
...
function main() {
  ...
  const impossibleRanges = mapImposibleRanges(sensors, beacons);
  const lostBeacon = findBeacon(impossibleRanges, sensors, beacons);
  console.log(lostBeacon.x * 4000000 + lostBeacon.y);
}
