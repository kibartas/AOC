...
the_time = None
min_results_counter = float('inf')
for sometime in every_space[(1, 1)]:
    print(sometime)
    the_time = sometime
    something = a_star((len(map[0])-2, len(map)-2), (1, 1, sometime))
    if something != 0:
        ...
        results_counter += current_node[2]
        if results_counter < min_results_counter:
            min_results_counter = results_counter
        break

result = min_results_counter + 1
