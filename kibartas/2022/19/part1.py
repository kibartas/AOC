...
def work(time_limit=24, blueprint=2, robot_counts=robot_counts, resource_counts=resource_counts):
    ...
    for i in range(4):
        if i == 0 and resource_counts[0] >= blueprints[blueprint][0] and robot_counts[i] <= blueprints[blueprint][-1] // 2 + 1:
            ...
            results.append(work(time_limit-1, blueprint, new_robot_counts, inner_new_resource_counts))
            counter += 1
        elif i == 3 and resource_counts[0] >= blueprints[blueprint][4] and resource_counts[2] >= blueprints[blueprint][5]:
